...
class RTFGeneratorEx extends Object {
    
    Dictionary<Object, Object> colorTable;
    int colorCount;
    Dictionary<Object, Object> fontTable;
    int fontCount;
    Dictionary<Object, Object> styleTable;
    int styleCount;
    ...
    static {
        ...
        Dictionary<Object, Object> textKeywordDictionary = RTFReaderEx.textKeywords;
        Enumeration<Object> keys = textKeywordDictionary.keys();
        Vector<Object> tempPairs = new Vector<Object>();
    ...
    static public void writeDocument(
        ...
        RTFGeneratorEx gen = new RTFGeneratorEx(to);
    ...
    public RTFGeneratorEx(
            OutputStream to) {
        colorTable = new Hashtable<Object, Object>();
        ...
        styleTable = new Hashtable<Object, Object>();
    ...
    private Integer findStyleNumber(
            AttributeSet a,
            String domain) {
        while (a != null) {
            if (a instanceof Style) {
                Integer aNum = (Integer) styleTable.get(a);
                if (aNum != null) {
                    if (domain == null || domain.equals(a.getAttribute(ConstantsEx.StyleType)))
                        return aNum;
                }
    ...
    static private Object attrDiff(
            MutableAttributeSet oldAttrs,
    ...
    static private boolean equalArraysOK(
            Object a,
            Object b) {
        Object[] aa, bb;
        if (a == b)
    ...
    public void writeRTFHeader() throws IOException {
        ...
        Enumeration<Object> fonts = fontTable.keys();
        String font;
        ...
        if (styleCount > 1) {
            ...
            Enumeration<Object> styles = styleTable.keys();
            while (styles.hasMoreElements()) {
                Style style = (Style) styles.nextElement();
                int styleNumber = ((Integer) styleTable.get(style)).intValue();
                writeBegingroup();
                String styleType = (String) style.getAttribute(ConstantsEx.StyleType);
                if (styleType == null)
                    styleType = ConstantsEx.STParagraph;
                if (styleType.equals(ConstantsEx.STCharacter)) {
                    writeControlWord("*");
                    writeControlWord("cs", styleNumber);
                } else if (styleType.equals(ConstantsEx.STSection)) {
                    writeControlWord("*");
                ...
                Style nextStyle = (Style) style.getAttribute(ConstantsEx.StyleNext);
                if (nextStyle != null) {
                ...
                Boolean hidden = (Boolean) style.getAttribute(ConstantsEx.StyleHidden);
                if (hidden != null && hidden.booleanValue())
                    writeControlWord("shidden");

                Boolean additive = (Boolean) style.getAttribute(ConstantsEx.StyleAdditive);
    ...
    void writeDocumentProperties(
        ...
        for (i = 0; i < RTFAttributesEx.attributes.length; i++) {
            RTFAttributeEx attr = RTFAttributesEx.attributes[i];
            if (attr.domain() != RTFAttributeEx.D_DOCUMENT)
                continue;
    ...
    protected void checkNumericControlWord(
            MutableAttributeSet currentAttributes,
    ...
    protected void checkControlWord(
            MutableAttributeSet currentAttributes,
            AttributeSet newAttributes,
            RTFAttributeEx word) throws IOException {
        Object parm;
    ...
    protected void checkControlWords(
            MutableAttributeSet currentAttributes,
            AttributeSet newAttributes,
            RTFAttributeEx words[],
            int domain) throws IOException {
        int wordIndex;
        int wordCount = words.length;
        for (wordIndex = 0; wordIndex < wordCount; wordIndex++) {
            RTFAttributeEx attr = words[wordIndex];
    ...
    void updateSectionAttributes(
            MutableAttributeSet current,
            AttributeSet newAttributes,
            boolean emitStyleChanges) throws IOException {
        if (emitStyleChanges) {
            Object oldStyle = current.getAttribute("sectionStyle");
            Object newStyle = findStyleNumber(newAttributes, ConstantsEx.STSection);
        ...
        checkControlWords(current, newAttributes, RTFAttributesEx.attributes, RTFAttributeEx.D_SECTION);
    }
    ...
    protected void resetSectionAttributes(
        ...
        int wordIndex;
        int wordCount = RTFAttributesEx.attributes.length;
        for (wordIndex = 0; wordIndex < wordCount; wordIndex++) {
            RTFAttributeEx attr = RTFAttributesEx.attributes[wordIndex];
    ...
    void updateParagraphAttributes(
        ...
        if (emitStyleChanges) {
            ...
            newStyle = findStyleNumber(newAttributes, ConstantsEx.STParagraph);
            if (oldStyle != null && oldStyle.equals(newStyle)) {
                if (oldStyle != null) {
        ...
        Object oldTabs = current.getAttribute(ConstantsEx.Tabs);
        Object newTabs = newAttributes.getAttribute(ConstantsEx.Tabs);
        if (oldTabs != newTabs) {
            if (oldTabs != null) {
                resetParagraphAttributes(current);
                oldTabs = null;
        ...
        if (oldStyle != null && oldStyle.equals(newStyle) && newStyle != null) {
            writeControlWord("s", ((Integer) newStyle).intValue());
            current.addAttribute("paragraphStyle", newStyle);
        }

        checkControlWords(current, newAttributes, RTFAttributesEx.attributes, RTFAttributeEx.D_PARAGRAPH);
        ...
        if (oldTabs != newTabs && newTabs != null) {
            ...
            current.addAttribute(ConstantsEx.Tabs, tabs);
        }
    }

    
    public void writeParagraphElement(
            Element el) throws IOException {
    ...
    protected void resetParagraphAttributes(
        ...
        currentAttributes.removeAttribute(ConstantsEx.Tabs);
    }
    ...
    void updateCharacterAttributes(
        ...
        if (updateStyleChanges) {
            ...
            Object newStyle = findStyleNumber(newAttributes, ConstantsEx.STCharacter);
            if (oldStyle != null && oldStyle.equals(newStyle)) {
                if (oldStyle != null) {
        ...
        checkControlWords(current, newAttributes, RTFAttributesEx.attributes, RTFAttributeEx.D_CHARACTER);
    ...
    String approximationForUnicode(
            char ch) {
    ...
    static int[] outputConversionFromTranslationTable(
            char[] table) {
    ...
    static int[] outputConversionForName(
            String name) throws IOException {
        char[] table = (char[]) RTFReaderEx.getCharacterSet(name);
        return outputConversionFromTranslationTable(table);
    }

    
    static protected int convertCharacter(
            int[] conversion,
            char ch) {
        int index;

...
class RTFReaderEx extends RTFParserEx {
	...
	Dictionary<Object, Object> fontTable;
	...
	static private Dictionary<Object, Object> straightforwardAttributes;
	static {
		straightforwardAttributes = RTFAttributesEx.attributesByKeyword();
	}

	private MockAttributeSetEx mockery;

	
	static Dictionary<Object, Object> textKeywords = null;
	static {
		textKeywords = new Hashtable<Object, Object>();
		textKeywords.put("\\", "\\");
		textKeywords.put("{", "{");
		textKeywords.put("}", "}");
		textKeywords.put(" ", "\u00A0"); 
	...
	static Dictionary<Object, Object> characterSets;
	static boolean useNeXTForAnsi = false;
	static {
		characterSets = new Hashtable<Object, Object>();
	}

	
	public RTFReaderEx(
			StyledDocument destination) {
		target = destination;
		parserState = new Hashtable<Object, Object>();
		fontTable = new Hashtable<Object, Object>();

		rtfversion = -1;

		mockery = new MockAttributeSetEx();
		documentAttributes = new SimpleAttributeSet();
	}

	
	public void handleBinaryBlob(
			byte[] data) {
	...
	public void begingroup() {
		...
		Dictionary<Object, Object> saveState = (Dictionary<Object, Object>) ((Hashtable<Object, Object>) parserState).clone();
		if (oldSaveState != null)
	...
	protected void setRTFDestination(
			Destination newDestination) {
		@SuppressWarnings("unchecked")
		Dictionary<Object, Object> previousState = (Dictionary<Object, Object>) parserState.get("_savedState");
	...
	public boolean handleKeyword(
		...
		if (keyword.equals("aftncn") || keyword.equals("aftnsep") || keyword.equals("aftnsepc") || keyword.equals("annotation")
				|| keyword.equals("atnauthor") || keyword.equals("atnicn") || keyword.equals("atnid") || keyword.equals("atnref")
				|| keyword.equals("atntime") || keyword.equals("atrfend") || keyword.equals("atrfstart") || keyword.equals("bkmkend")
				|| keyword.equals("bkmkstart") || keyword.equals("datafield") || keyword.equals("do") || keyword.equals("dptxbxtext")
				|| keyword.equals("falt") || keyword.equals("field") || keyword.equals("file") || keyword.equals("filetbl")
				|| keyword.equals("fname") || keyword.equals("fontemb") || keyword.equals("fontfile") || keyword.equals("footer")
				|| keyword.equals("footerf") || keyword.equals("footerl") || keyword.equals("footerr") || keyword.equals("footnote")
				|| keyword.equals("ftncn") || keyword.equals("ftnsep") || keyword.equals("ftnsepc") || keyword.equals("header")
				|| keyword.equals("headerf") || keyword.equals("headerl") || keyword.equals("headerr") || keyword.equals("keycode")
				|| keyword.equals("nextfile") || keyword.equals("object") || keyword.equals("pict") || keyword.equals("pn")
				|| keyword.equals("pnseclvl") || keyword.equals("pntxtb") || keyword.equals("pntxta") || keyword.equals("revtbl")
				|| keyword.equals("rxe") || keyword.equals("tc") || keyword.equals("template") || keyword.equals("txe") || keyword.equals("xe")) {
	...
	private void setTargetAttribute(
			String name,
			Object value) {
		...
		if (set != null) {
			...
			if (!name.equals("ansi")) {
				try {
					...
					throw new InternalError("RTFReaderEx: Unable to find character set resources (" + e + ")");
				}
			}
		}

		setTargetAttribute(ConstantsEx.RTFCharacterSet, name);
	}

	
	public static void defineCharacterSet(
			String name,
			char[] table) {
		if (table.length < 256)
	...
	public static Object getCharacterSet(
		...
		if (set == null) {
			...
			charsetStream = (InputStream) java.security.AccessController.doPrivileged(new java.security.PrivilegedAction() {
				public Object run() {
					return RTFReaderEx.class.getResourceAsStream("charsets/" + name + ".txt");
				}
			});
	...
	static char[] readCharset(
			InputStream strm) throws IOException {
	...
	static char[] readCharset(
			java.net.URL href) throws IOException {
	...
	interface Destination {
		...
		void endgroup(
				Dictionary<Object, Object> oldState);
	...
	class StylesheetDestination extends DiscardingDestination implements Destination {
		Dictionary<Object, Object> definedStyles;

		
		public StylesheetDestination() {
			definedStyles = new Hashtable<Object, Object>();
		}
		...
		public void close() {
			Vector<Object> chrStyles, pgfStyles, secStyles;
			chrStyles = new Vector<Object>();
			pgfStyles = new Vector<Object>();
			secStyles = new Vector<Object>();
			...
			while (styles.hasMoreElements()) {
				...
				String stype = (String) defined.getAttribute(ConstantsEx.StyleType);
				Vector<Object> toSet;
				if (stype.equals(ConstantsEx.STSection)) {
					toSet = secStyles;
				} else if (stype.equals(ConstantsEx.STCharacter)) {
					toSet = chrStyles;
				} else {
					toSet = pgfStyles;
				}
		...
		class StyleDefiningDestination extends AttributeTrackingDestination implements Destination {
			...
			public Style realize() {
				...
				if (characterStyle) {
					...
					realizedStyle.addAttribute(ConstantsEx.StyleType, ConstantsEx.STCharacter);
				} else if (sectionStyle) {
					realizedStyle.addAttributes(currentSectionAttributes());
					realizedStyle.addAttribute(ConstantsEx.StyleType, ConstantsEx.STSection);
				} else {
					realizedStyle.addAttributes(currentParagraphAttributes());
					realizedStyle.addAttribute(ConstantsEx.StyleType, ConstantsEx.STParagraph);
				}
				...
				if (next != null)
					realizedStyle.addAttribute(ConstantsEx.StyleNext, next);
				realizedStyle.addAttribute(ConstantsEx.StyleAdditive, Boolean.valueOf(additive));
				realizedStyle.addAttribute(ConstantsEx.StyleHidden, Boolean.valueOf(hidden));
	...
	abstract class AttributeTrackingDestination implements Destination {
		...
		abstract public void handleText(
				String text);
		...
		public boolean handleKeyword(
...

			{
				...
				if (item != null) {
					RTFAttributeEx attr = (RTFAttributeEx) item;
					boolean ok;

					switch (attr.domain()) {
					case RTFAttributeEx.D_CHARACTER:
						ok = attr.set(characterAttributes);
						break;
					case RTFAttributeEx.D_PARAGRAPH:
						ok = attr.set(paragraphAttributes);
						break;
					case RTFAttributeEx.D_SECTION:
						ok = attr.set(sectionAttributes);
						break;
					case RTFAttributeEx.D_META:
						mockery.backing = parserState;
						ok = attr.set(mockery);
						mockery.backing = null;
						break;
					case RTFAttributeEx.D_DOCUMENT:
		...
		MutableAttributeSet currentParagraphAttributes() {
			...
			if (tabs == null) {
				@SuppressWarnings("unchecked")
				Dictionary<Object, Object> workingTabs = (Dictionary<Object, Object>) parserState.get("_tabs");
				if (workingTabs != null) {
			...
			if (tabs != null)
				bld.addAttribute(ConstantsEx.Tabs, tabs);
		...
		protected void resetCharacterAttributes() {
			...
			while (attributes.hasMoreElements()) {
				RTFAttributeEx attr = (RTFAttributeEx) attributes.nextElement();
				if (attr.domain() == RTFAttributeEx.D_CHARACTER)
		...
		protected void resetParagraphAttributes() {
			...
			while (attributes.hasMoreElements()) {
				RTFAttributeEx attr = (RTFAttributeEx) attributes.nextElement();
				if (attr.domain() == RTFAttributeEx.D_PARAGRAPH)
					attr.setDefault(characterAttributes);
			}
		}
		...
		protected void resetSectionAttributes() {
			...
			while (attributes.hasMoreElements()) {
				RTFAttributeEx attr = (RTFAttributeEx) attributes.nextElement();
				if (attr.domain() == RTFAttributeEx.D_SECTION)
					attr.setDefault(characterAttributes);
			}
